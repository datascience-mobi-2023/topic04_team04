...
subset_mean_temp = function(start, end, mean_tempvec, month_dates){
  # input start and end as "YYYY-MM-DD"
...
Plotting the timeframe of min and max temperature
...
## ARIMA-modeling Dengue cases

```{r}
#creating a time series of the total dengue cases 
Ts_thai <- ts(total_thailand$dengue_cases, frequency = 12, start = c(2006, 1))
TSstudio::ts_plot(Ts_thai)

library(tseries)
adf.test(Ts_thai) # p-value (0.01) < 0.05 indicates the TS is stationary 
kpss.test(Ts_thai, null = c("Level", "Trend")) # p-value (0.1) > 0.05 indicates stationarity
...
### Evaltuating the fit of the model
...
based on the auto ARIMA a forecast is created for 4 years and plotted with the time series of total dengue cases
...
A forecast is created based on the model "by hand"
...
To compare the forecast to the actual data, the timeseries is cropped after 2016. This time series is used to forcast the years 2017 - 2020 and compared to the actual data
...
## Decomposition of temperature and cases
...
# creating a timeseries of temperature 
...
### Comparing the trend of temperature and cases (additive decomposition)
...
### Comparing the trend of temperature and cases (seasonal decomposition)

```{r}
case_seasonal = decomposed_thai_add$seasonal
temp_seasonal = decomposed_thai_temp_add$seasonal
...
## Mean of dengue cases and temperature of whole thailand per month

```{r}
# mean dengue cases for the whole country per month
...
# mean temperature for the whole country per month
...
# plotting mean temperature and dengue cases
...
# computing and plotting the difference between mean and median of dengue cases
...
# for only plotting the dengue case plots:
# gridExtra::grid.arrange(plot_mean_dengue, plot_median_dengue, plot_dif_dengue, nrow=3)

# computing and plotting the difference between mean and median of temperature
...
# for only plotting the temperature plots:
...
#### Plotting dengue cases for Bangkok
...
smoothed_curve <- ss(clean_data_total$temperature, clean_data_total$incidence, nknots = 16)
...
## Average incidence for each month map - log(incidence)

```{r}

f_map_mean_inc.m.p = function(period, areas){
  map_mean_inc.m.p <- data.frame(incidence = numeric(), time_column = as.Date(character()), Reporting_areas = character())
  for (p in areas) {
    provinz = map_data_total[map_data_total$Reporting_areas == p,]
    p_month = provinz[provinz$time_column %in% period,]
    m_m_p = data.frame(incidence = mean(log(p_month$incidence + 1), na.rm = TRUE), time_column = p_month$time_column[1], Reporting_areas = p_month$Reporting_areas[1])
    map_mean_inc.m.p = rbind(map_mean_inc.m.p, m_m_p)
  }
  return(map_mean_inc.m.p)
}

map_mean_inc.m.p = f_map_mean_inc.m.p(period = seq(from = as.Date("2006-01-01"), to = as.Date("2020-12-01"), by = "month"), areas = unique(map_data_total$Reporting_areas))
district_sf = st_read("gadm36_THA_shp/gadm36_THA_1.shp")
district_sf$incidence <- map_mean_inc.m.p$incidence

ggplot2::ggplot() +
  geom_sf(data = district_sf, aes(fill = incidence)) +
  scale_fill_gradientn(colors = rev(terrain.colors(100))) + labs(fill = "Incidence") + labs(title = "Average incidences 2006 - 2020")


f_map_mean_temp.m.p = function(period, areas){
  map_mean_temp.m.p <- data.frame(temperature = numeric(), time_column = as.Date(character()), Reporting_areas = character())
  for (p in areas) {
    provinz = map_data_total[map_data_total$Reporting_areas == p,]
    p_month = provinz[provinz$time_column %in% period,]
    m_m_p = data.frame(temperature = mean(p_month$temperature, na.rm = TRUE), time_column = p_month$time_column[1], Reporting_areas = p_month$Reporting_areas[1])
    map_mean_temp.m.p = rbind(map_mean_temp.m.p, m_m_p)
  }
  return(map_mean_temp.m.p)
}

map_mean_temp.m.p = f_map_mean_temp.m.p(period = seq(from = as.Date("2006-01-01"), to = as.Date("2020-12-01"), by = "month"), areas = unique(map_data_total$Reporting_areas))
district_sf = st_read("gadm36_THA_shp/gadm36_THA_1.shp")
district_sf$temperature <- map_mean_temp.m.p$temperature

ggplot2::ggplot() +
  geom_sf(data = district_sf, aes(fill = temperature)) +
  scale_fill_gradientn(colors = rev(terrain.colors(100))) + labs(fill = "Temperature") + labs(title = "Average temperature 2006 - 2020")
```

## Average temperature 2006 to 2020 map

```{r}

f_map_mean_inc.p.t = function(areas){
  map_mean_inc.p.t <- data.frame(incidence = numeric(), time_column = as.Date(character()), Reporting_areas = character(), temperature = numeric())
  for (p in areas) {
      provinz = map_data_total[map_data_total$Reporting_areas == p,]
      m_p.t = data.frame(time_column = provinz$time_column[1], Reporting_areas = provinz$Reporting_areas[1],incidence = mean(provinz$incidence, na.rm = TRUE), temperature = mean(provinz$temperature, na.rm = TRUE))
      map_mean_inc.p.t = rbind(map_mean_inc.p.t, m_p.t)
  }
  return(map_mean_inc.p.t)
} 

map_mean_inc.p.t = f_map_mean_inc.p.t(areas = unique(map_data_total$Reporting_areas))
district_sf.t <- st_read("gadm36_THA_shp/gadm36_THA_1.shp")
district_sf.t$temperature <- map_mean_inc.p.t$temperature


ggplot() +
  geom_sf(data = district_sf.t, aes(fill = temperature)) +
 scale_fill_gradientn(colors = rev(terrain.colors(100)), limits = c( 22, 31)) + labs(fill = "temperature") +
  labs(title = "Average Temperatures for 2006-2020 ")# limits = c( 0, 22.75)
```
