...
ggplot2::ggplot() +
  geom_line(data = mean_dengue, aes(x = time_column, y = dengue_cases)) +
  labs(x = "Zeit", y = "Dengue-Fälle") +
  geom_line(data = mean_temp, aes(x = time_column, y = temperature), color = "red") + 
  scale_y_continuous(sec.axis = sec_axis(trans = ~ rescale(., to = range(mean_temp$temperature)), name = "Temperatur"), name = "Dengue-Fälle")
...
data <- read_excel("C:/Users/thadl/OneDrive/Dokumente/Bioinfo/Klimaprojekt/Daten/Data for modeling.xlsx")
View(data)
newdata = data.frame(data)
attach(newdata)
model1 <- gam(Abun1~s(Maxtemp, k=4), family="quasipoisson")
plot.gam(model1)
summary(model1)
model2 <- gam(Abun2~s(Maxtemp, k=4), family="quasipoisson")
plot.gam(model2)
summary(model2)
k.check(model1)
gam.check(model1)

linear_model <- gam(Abun1~Maxtemp)
smooth_model <- gam(Abun1~s(Maxtemp))
