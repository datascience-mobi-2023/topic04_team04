...
#create data frame for south west monsoon season
```{r}
vec_month_swmonsoon = c("Jun", "Jul", "Aug", "Sep")
data_swmonsoon = data_total[data_total$month %in% vec_month_swmonsoon,]
...
#create data frame for north west monsoon season
```{r}
vec_month_nwmonsoon = c("Oct", "Nov", "Dec", "Jan")
data_nwmonsoon = data_total[data_total$month %in% vec_month_nwmonsoon,]


```

#create data frame for pre monsoon season
```{r}
vec_month_premonsoon = c("Feb", "Mar", "Apr", "May")
data_premonsoon = data_total[data_total$month %in% vec_month_premonsoon,]

compare_season_temp = c(nwmonsoon = sd(data_nwmonsoon$temperature, na.rm = T), swmonsoon =  sd(data_swmonsoon$temperature, na.rm = T), premonsoon = sd(data_premonsoon$temperature, na.rm = T))
compare_season_temp
compare_season_inc = c(nwmonsoon = sd(data_nwmonsoon$incidence, na.rm = T), swmonsoon =  sd(data_swmonsoon$incidence, na.rm = T), premonsoon = sd(data_premonsoon$incidence, na.rm = T))
compare_season_inc
compare_season_temp = c(nwmonsoon = mean(data_nwmonsoon$temperature, na.rm = T), swmonsoon =  mean(data_swmonsoon$temperature, na.rm = T), premonsoon = mean(data_premonsoon$temperature, na.rm = T))
compare_season_temp
compare_season_inc = c(nwmonsoon = mean(data_nwmonsoon$incidence, na.rm = T), swmonsoon =  mean(data_swmonsoon$incidence, na.rm = T), premonsoon = mean(data_premonsoon$incidence, na.rm = T))
compare_season_inc


sd_areas = c()
for (i in r_areas2) {
  sd_areas = c(sd_areas, sd(data_total$incidence[data_total$Reporting_areas == i], na.rm = T))
}
sd_areas
...
mean_areas = c()
for (i in r_areas2) {
  mean_areas = c(mean_areas, mean(data_total$temperature[data_total$Reporting_areas == i], na.rm = T))
}
sd(mean_areas)
  
sd(data_total$incidence, na.rm = T)
sd(data_total$temperature, na.rm = T)/mean(data_total$temperature, na.rm = T)
...
map_mean_inc.y.p = f_map_mean_inc.y.p(period = 2012,areas = unique(map_data_total$Reporting_areas))
district_sf = st_read("gadm36_THA_shp/gadm36_THA_1.shp")
district_sf$incidence <- map_mean_inc.y.p$incidence

ggplot2::ggplot() +
  geom_sf(data = district_sf, aes(fill = incidence)) +
  scale_fill_gradientn(colors = rev(terrain.colors(100)), limits = c(0,22.8)) + labs(fill = "Incidence") + labs(title = "Average incidences for 2012")
