...
```{r}
mu <- round(mean(data_total$incidence, na.rm = TRUE), digits = 0)
size <- round(var(data_total$incidence, na.rm = TRUE) / (mu - 1), digits = 0)
...
curve(dnbinom(round(x, digits = 0), size = size, mu = mu), 
      from = round(min(data_total$incidence, na.rm = TRUE), digits = 0),
      to = round(max(data_total$incidence, na.rm = TRUE), digits = 0),
