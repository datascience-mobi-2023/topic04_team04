...
f_map_mean_inc.p = function(areas){
  map_mean_inc.p <- data.frame(incidence = numeric(), time_column = as.Date(character()), Reporting_areas = character())
  for (p in areas) {
...
map_mean_inc.p = f_map_mean_inc.p(areas = r_areas)
district_sf <- st_read("gadm36_THA_shp/gadm36_THA_1.shp")
district_sf$incidence <- map_mean_inc.p$incidence

ggplot() +
  geom_sf(data = district_sf, aes(fill = incidence)) +
  scale_fill_gradient(low = "white", high = "green") + labs(fill = "Incidence")
