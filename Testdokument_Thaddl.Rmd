...
f_map_mean_inc.m.p = function(period, areas){
  ...
  for (p in areas) {
    ...
    m_m_p = data.frame(incidence = mean(p_month$incidence, na.rm = TRUE), time_column = p_month$time_column[1], Reporting_areas = p_month$Reporting_areas[1])
    map_mean_inc.m.p = rbind(map_mean_inc.m.p, m_m_p)
  }
  return(map_mean_inc.m.p)
}

map_mean_inc.m.p = f_map_mean_inc.m.p(period = seq(from = as.Date("2007-01-01"), to = as.Date("2007-12-01"), by = "month"), areas = unique(map_data_total$Reporting_areas))
district_sf = st_read("gadm36_THA_shp/gadm36_THA_1.shp")
...
map_mean_temp.m.p = f_map_mean_temp.m.p(period = seq(from = as.Date("2007-01-01"), to = as.Date("2007-12-01"), by = "month"), areas = unique(map_data_total$Reporting_areas))
district_sf = st_read("gadm36_THA_shp/gadm36_THA_1.shp")
