...
kpss.test(Ts_thailand, null = c("Level", "Trend"))
...
Ts_thailand_seasdiff <- diff(Ts_thailand, lag = frequency(Ts_thailand), differences = 1)  # seasonal differencing 
plot(Ts_thailand_seasdiff, type="l", main = "Seasonally Differenced")  

# 2md Seasonal Differencing
stationaryTs_thailand <- diff(Ts_thailand_seasdiff, differences = 1)
plot(stationaryTs_thailand, type = "l", main = "Differenced and Stationary")  # --> USUALLY you do not difference more than TWO times 

acfRes_stationary <- acf(stationaryTs_thailand)
pacfRes_stationary <- pacf(stationaryTs_thailand)
```
...
for (p in p_range) {
  for (d in d_range) {
    for (q in q_range) {
      ...
      # Fit the ARIMA model
      model <- arima(Ts_thailand, order = c(p, d, q)) 
...
best_model <- arima(Ts_thailand, order = c(p_best, d_best, q_best))  # Replace with the best order values
best_model 
...
#auto_thailand_stat = auto.arima(stationaryTs_thailand)
#auto_thailand_stat
...
library(forecast)
horizon_thailand <- 12 
forecasted_best_thailand <- forecast(best_model, h = horizon_thailand)

forecasted_best_Ts_thailand <- forecasted_best_thailand$mean
forecasted_best_Ts_thailand  <- ts(forecasted_best_Ts_thailand, frequency = 12, start = c(2021, 1) )
plot(forecasted_best_Ts_thailand)
```

```{r}
forecastDf_best_thailand <- as.data.frame(forecasted_best_thailand)
forecastDf_best_thailand$time <- time(forecasted_best_Ts_thailand)
names(forecastDf_best_thailand) <- c("point", "lo_80", "hi_80" , "lo_95", "hi_95", "time")
```
...
ggplot() +
  geom_line(data = Ts_thailand, aes(x = time(Ts_thailand), y = Ts_thailand), color = "black") +
  geom_ribbon(data = forecastDf_best_thailand, aes(x = time, ymin = lo_95, ymax = hi_95), fill = "gray80") +
  geom_line(data = forecastDf_best_thailand, aes(x = time, y = point), color = "blue") +
  labs(title = "Time Series Forecast") +
  xlab("Year") +
  ylab("Dengue Cases") +
  theme_minimal()
